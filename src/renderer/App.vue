<template>
  ...
  <div class="app">
    ...
    <div
      v-if="!isOnline"
      class="connection-banner offline"
    ...
    <div
      v-else-if="failedQueueCount > 0"
      class="retry-banner"
      @click="handleRetryQueue"
    >
      <span v-if="isRetrying">Retrying...</span>
      <span v-else
        >{{ failedQueueCount }} failed track{{
          failedQueueCount === 1 ? "" : "s"
        }}
        — tap to retry</span
      >
    </div>
...
const ensureDevicePath = () => {
  if (!preferences.devicePath || !path.value) {
    if (!missingDevicePathNotified.value) {
      showErrorPopup(
        "Device path not set. Please select your iPod in Settings."
      );
...
const clearPlayCounts = async (resetScrobbled) => {
  ...
  // Try to delete the PlayCounts file (may not exist)
  const deletedFile = await window.ipc.deletePlaycount(path.value);
  if (deletedFile) {
    console.log("Deleted Playcounts");
  }

  // Always clear the local state and ledger
  logDebug("clearPlayCounts clearing local state");
  await clearTracklist();
...
const scrobbleNewTracks = async () => {
  ...
  if (!networkStatus.online) {
    showErrorPopup(
      `Cannot scrobble: ${networkStatus.error || "No internet connection"}. Please check your connection and try again.`
    );
    return;
  }
...
async function handleTestConnection() {
  ...
  try {
    ...
    if (result.online) {
      ...
      showErrorPopup(
        `Still offline: ${result.error || "No internet connection"}`
      );
    }
  } finally {
...
async function handleRetryQueue() {
  ...
  if (!networkStatus.online) {
    showErrorPopup(
      `Cannot retry: ${networkStatus.error || "No internet connection"}. Please check your connection and try again.`
    );
    return;
  }
...
function checkForFailedQueue() {
  ...
  if (queue.length > 0) {
    ...
    logDebug("checkForFailedQueue showing prompt", {
